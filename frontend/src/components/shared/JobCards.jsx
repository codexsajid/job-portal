...
const JobCards = ({ job }) => {
    ...
    return (
        <div className='border border-gray-300 bg-white rounded-xl p-3 sm:p-4 shadow-sm'>
            <div className='flex justify-between items-center'>
                <p className='text-xs sm:text-sm text-gray-500'>{dayAgoFunction(job?.createdAt) == 0 ? "Today" : dayAgoFunction(job?.createdAt)} days ago</p>
                <Button variant='outline' className="rounded-full" size='icon'><Bookmark size={16} /></Button>
            </div>
            ...
            <div>
                ...
                <p className='text-gray-600 text-xs sm:text-sm'>{job?.description.substring(0, 80)}....</p>
            </div>
